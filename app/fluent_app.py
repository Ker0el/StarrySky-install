...
# ===== 新增功能翻译键（去重/批量入库/备份恢复/启动Steam/更新开关） =====
_NEW_FEATURE_TEXTS = {
    "zh_CN": {
        "already_installed": "AppID {0} 已入库（{1}），已跳过",
        "skip_duplicate_title": "跳过重复入库",
        "dup_status_record": "记录中",
        "dup_status_files": "清单文件已存在",
        "batch_install": "批量入库",
        "select_all": "全选",
        "batch_no_selection": "请先勾选要批量入库的游戏",
        "batch_skipped": "已跳过 {0} 个已入库的游戏",
        "batch_done": "批量入库完成：成功 {0} 个，失败 {1} 个",
        "batch_failed": "批量入库失败",
        "installing_in_progress": "当前有入库任务进行中，请稍后再试",
        "clear_installed": "清空已入库",
        "clear_installed_title": "确认清空",
        "clear_installed_message": "确定要清空所有已入库记录吗？\n\n此操作不可撤销。",
        "clear_manifests_opt": "同时删除清单文件（depotcache 下的 .manifest）",
        "clear_success": "已清空",
        "clear_failed": "清空失败",
        ...
        "steam_not_found": "未找到 Steam 或 steam.exe，请在设置中检查 Steam 路径",
        "backup_restore": "备份与恢复",
        "backup": "备份",
        "restore": "恢复",
        "backup_hint": "将已入库记录和 Steam 清单文件打包为 zip",
        "backup_success": "备份成功",
        "backup_failed": "备份失败",
        "restore_success": "恢复成功，共恢复 {0} 个文件",
        "restore_failed": "恢复失败",
        "restore_confirm_title": "确认恢复",
        "restore_confirm_message": "恢复将覆盖当前的已入库记录和清单文件（当前文件会保留 .bak 备份）。\n\n确定继续吗？",
        "auto_check_update": "自动检查更新",
        "auto_check_update_hint": "启动时自动检查新版本",
        "source_label": "来源",
        "source_unknown": "未知",
        "source_auto": "自动",
    ...
    "en_US": {
        ...
        "skip_duplicate_title": "Duplicate skipped",
        "dup_status_record": "in records",
        "dup_status_files": "manifest files exist",
        "batch_install": "Batch Install",
        "select_all": "Select All",
        "batch_no_selection": "Please check the games to install first",
        "batch_skipped": "Skipped {0} already-installed game(s)",
        "batch_done": "Batch done: {0} succeeded, {1} failed",
        "batch_failed": "Batch install failed",
        "installing_in_progress": "An install task is in progress, try again later",
        "clear_installed": "Clear Library",
        "clear_installed_title": "Confirm Clear",
        "clear_installed_message": "Clear all installed records?\n\nThis cannot be undone.",
        "clear_manifests_opt": "Also delete manifest files (.manifest in depotcache)",
        ...
        "steam_not_found": "Steam or steam.exe not found, check the Steam path in Settings",
        "backup_restore": "Backup & Restore",
        "backup": "Backup",
        "restore": "Restore",
        "backup_hint": "Package library records and Steam manifest files into a zip",
        "backup_success": "Backup successful",
        "backup_failed": "Backup failed",
        "restore_success": "Restore successful, {0} file(s) restored",
        "restore_failed": "Restore failed",
        "restore_confirm_title": "Confirm Restore",
        "restore_confirm_message": "Restoring will overwrite current records and manifest files (current files will be kept as .bak backups).\n\nContinue?",
        "auto_check_update": "Auto-check Updates",
        "auto_check_update_hint": "Check for new versions on startup",
        "source_label": "Source",
...
for _lang, _keys in _NEW_FEATURE_TEXTS.items():
    if _lang in TEXTS:
        TEXTS[_lang].update(_keys)
for _lang in TEXTS:
    if _lang not in _NEW_FEATURE_TEXTS:
        TEXTS[_lang].update(_NEW_FEATURE_TEXTS["zh_CN"])
...
# ===== 已入库记录管理 =====
INSTALLED_RECORDS_PATH = APP_ROOT / 'config' / 'installed_games.json'


def _load_installed_records() -> list:
    """加载已入库记录: [{appid, name, source, installed_at, dlcs:[appid,...]}]"""
    try:
        if INSTALLED_RECORDS_PATH.exists():
            import json
            with open(INSTALLED_RECORDS_PATH, 'r', encoding='utf-8') as f:
                data = json.load(f)
...
def _save_installed_records(records: list):
    """保存已入库记录"""
    try:
        import json
        INSTALLED_RECORDS_PATH.parent.mkdir(parents=True, exist_ok=True)
        with open(INSTALLED_RECORDS_PATH, 'w', encoding='utf-8') as f:
            json.dump(records, f, indent=2, ensure_ascii=False)
    except Exception:
        pass


def _installed_appid_set(records=None) -> set:
    """所有已入库 appid（含记录内的 DLC appid）"""
    if records is None:
    ...
    for rec in records:
        ...
        if aid.isdigit():
            ids.add(aid)
        for d in (rec.get('dlcs') or []):
            did = str(d.get('appid', d)) if isinstance(d, dict) else str(d)
            did = did.strip()
            if did.isdigit():
                ids.add(did)
    return ids
...
def _remove_installed_record(appid):
    """按 appid 移除已入库记录"""
    try:
        records = _load_installed_records()
        filtered = [r for r in records if str(r.get('appid', '')) != str(appid)]
        if len(filtered) != len(records):
            _save_installed_records(filtered)
    except Exception:
        pass
...
def _get_steam_path_sync():
    """同步获取 Steam 路径（配置优先，回退注册表）"""
    try:
        config_path = APP_ROOT / 'config' / 'config.json'
        if config_path.exists():
            import json
            with open(config_path, 'r', encoding='utf-8') as f:
                config = json.load(f)
            custom = config.get("Custom_Steam_Path", "").strip()
            if custom:
                return Path(custom)
    except Exception:
        pass
...
def _manifest_dirs(steam_path):
    """清单文件目录列表（存在则返回）"""
    dirs = []
    for sub in ('depotcache', 'config/depotcache', 'appcache/depotcache'):
        d = steam_path / sub
        if d.exists():
            dirs.append(d)
    return dirs
...
def _get_existing_install_status(appid):
    """检查 appid 是否已入库。返回 'record'（记录中）/'files'（清单文件已存在）/None"""
    if not appid or not str(appid).isdigit():
        return None
    appid = str(appid)
    if appid in _installed_appid_set():
        return 'record'
    steam_path = _get_steam_path_sync()
    if steam_path:
        for d in _manifest_dirs(steam_path):
            try:
                for f in os.listdir(d):
                    if f.startswith(appid + "_") and f.endswith(".manifest"):
                        return 'files'
            except Exception:
                pass
    return None
...
# 来源显示名映射（值以 source_ 开头的是多语言 key）
_SOURCE_DISPLAY = {
    "auto": "source_auto",
    "cysaw": "Cysaw",
    "gmrc": "GMRC",
    "walftech": "Walftech",
    "sudama": "Sudama",
    "buqiuren": "source_buqiuren",
    "MHub": "MHub",
    "github_auiowu": "source_github",
    "steamautocracks_v2": "SWA V2",
    "steamautocracks_v1": "SWA V1",
    "sac-other": "source_sac_other",
}
...
def _source_display_name(source):
    ...
    s = str(source)
    if s.startswith("custom_zip_"):
        return tr("source_custom")
    display = _SOURCE_DISPLAY.get(s)
    if display is None:
        return s
    return tr(display) if display.startswith("source_") else display


def _zip_entry_target(name, steam_path, rec_path):
    """备份 zip 条目 → 恢复目标路径。白名单映射 + 路径穿越/绝对路径检查，不匹配返回 None"""
    if name.endswith('/') or '..' in name or name.startswith('/') or ':' in name:
        return None
    if name == 'installed_games.json':
        return rec_path
    if name.startswith('depotcache/'):
    ...
    if name == 'config.vdf':
        return steam_path / 'config' / 'config.vdf' if steam_path else None
    return None
...
class GameCard(CoverCard):
    ...
    def __init__(self, appid, game_name, source_type, parent=None, mode="auto", source=""):
        ...
        self.vBoxLayout.addWidget(self.sourceLabel, 0, Qt.AlignmentFlag.AlignVCenter)
        if self.modeLabel:
...
class GameCardGrid(CoverCard):
    ...
    def __init__(self, appid, game_name, source_type, parent=None, mode="auto", source=""):
        ...
        self.vBoxLayout.addSpacing(3)
        self.vBoxLayout.addWidget(self.sourceLabel, 0, Qt.AlignmentFlag.AlignHCenter)
...
class ClearInstalledDialog(MessageBoxBase):
    """清空已入库确认对话框（可勾选同时删除清单文件）"""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.titleLabel = TitleLabel(tr("clear_installed_title"), self)
        self.msgLabel = BodyLabel(tr("clear_installed_message"), self)
        self.msgLabel.setWordWrap(True)
        self.delete_manifests_check = CheckBox(tr("clear_manifests_opt"), self)
...
class HomePage(ScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        # 启动 Steam 按钮
        self.launch_steam_btn = PushButton(tr("launch_steam"), self)
        self.launch_steam_btn.setIcon(FluentIcon.POWER_BUTTON)
        ...
        self.clear_installed_btn.setFixedHeight(32)
        self.clear_installed_btn.clicked.connect(self.on_clear_installed_clicked)
    ...
    def on_launch_steam_clicked(self):
        """启动 Steam"""
        self.launch_steam_btn.setEnabled(False)
        ...
        async def _launch():
            ...
            if running:
                return {"running": True}
            if not steam_path or not (steam_path / 'steam.exe').exists():
                return {"error": "not_found"}
            subprocess.Popen([str(steam_path / 'steam.exe')], creationflags=subprocess.DETACHED_PROCESS, close_fds=True)
            return {"launched": True}

        _replace_worker(getattr(self, 'launch_steam_worker', None))
        self.launch_steam_worker = AsyncWorker(_launch())
        self.launch_steam_worker.result_ready.connect(self.on_launch_steam_complete)
        self.launch_steam_worker.error.connect(self.on_launch_steam_error)
        self.launch_steam_worker.finished.connect(self.launch_steam_worker.deleteLater)
        self.launch_steam_worker.start()

    @pyqtSlot(object)
    def on_launch_steam_complete(self, result):
        """启动 Steam 完成"""
        self.launch_steam_btn.setEnabled(True)
        if result.get("running"):
            InfoBar.info(
                title=tr("launch_steam"),
                content=tr("steam_running"),
                parent=self.window(),
                position=InfoBarPosition.TOP,
                duration=2500
            )
            ...
            InfoBar.error(
                title=tr("steam_launch_failed"),
                content=tr("steam_not_found"),
                parent=self.window(),
                position=InfoBarPosition.TOP,
                duration=4000
            )
    ...
    @pyqtSlot(str)
    def on_launch_steam_error(self, error):
        """启动 Steam 失败"""
        self.launch_steam_btn.setEnabled(True)
        InfoBar.error(
            title=tr("steam_launch_failed"),
    ...
    def on_clear_installed_clicked(self):
        """清空已入库"""
        dialog = ClearInstalledDialog(self.window())
        if not dialog.exec():
            return

        self.clear_installed_btn.setEnabled(False)

        async def _clear():
            deleted_manifests = 0
            if dialog.delete_manifests_check.isChecked():
                steam_path = _get_steam_path_sync()
                if steam_path:
                    for d in _manifest_dirs(steam_path):
                        try:
                            for f in list(d.glob("*.manifest")):
                                try:
                                    f.unlink()
                                    deleted_manifests += 1
                                except Exception:
                            ...
                            pass
            # 清空已入库记录
            _save_installed_records([])
            return {"manifests": deleted_manifests}

        _replace_worker(getattr(self, 'clear_worker', None))
        self.clear_worker = AsyncWorker(_clear())
        self.clear_worker.result_ready.connect(self.on_clear_complete)
    ...
    def on_clear_complete(self, result):
        """清空完成"""
        self.clear_installed_btn.setEnabled(True)
        content = tr("clear_success")
        if result.get("manifests"):
            content += f"（删除清单文件 {result['manifests']} 个）"
        InfoBar.success(
            title=tr("clear_installed"),
            content=content,
            parent=self.window(),
            position=InfoBarPosition.TOP,
            duration=3000
    ...
    @pyqtSlot(str)
    def on_clear_error(self, error):
        """清空失败"""
        self.clear_installed_btn.setEnabled(True)
        InfoBar.error(
            title=tr("clear_failed"),
            content=error,
            parent=self.window(),
            position=InfoBarPosition.TOP
        )
    ...
    def on_games_loaded(self, result):
        ...
        try:
            ...
            # 构建已入库记录映射（appid -> 记录）
            self._record_map = {str(r.get('appid', '')): r for r in _load_installed_records()}
    ...
    def display_games(self, games_data):
        ...
        for source_type, game in sorted_games:
            ...
            # 已入库记录中的来源
            source = (getattr(self, '_record_map', {}).get(str(appid)) or {}).get('source', '')

            # 根据视图模式创建不同类型的卡片
            if self.current_view_mode == "grid":
                card = GameCardGrid(appid, game_name, source_type, self, mode, source)
            else:
                card = GameCard(appid, game_name, source_type, self, mode, source)
    ...
    def on_delete_complete(self, result, appid):
        """删除完成"""
        if result.get('success'):
            # 同步移除已入库记录，避免去重拦截重新入库
            _remove_installed_record(appid)
            InfoBar.success(
...
class SearchResultCard(CoverCard):
    ...
    def __init__(self, appid, game_name, parent=None):
        ...
        self.checkBox.setToolTip(tr("batch_install"))
        ...
        self.hBoxLayout.addLayout(self.vBoxLayout)
        self.hBoxLayout.addStretch(1)
        self.hBoxLayout.insertWidget(0, self.checkBox, 0, Qt.AlignmentFlag.AlignVCenter)
...
class SearchResultCardGrid(CoverCard):
    ...
    def __init__(self, appid, game_name, parent=None):
        ...
        btn_row.addWidget(self.checkBox)
...
class SearchPage(ScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        # 批量入库
        self.select_all_check = CheckBox(tr("select_all"), self)
        self.select_all_check.stateChanged.connect(self.on_select_all_changed)
        progress_layout.addWidget(self.select_all_check)
        self.batch_install_btn = PrimaryPushButton(tr("batch_install"), self)
        self.batch_install_btn.setIcon(FluentIcon.CLOUD_DOWNLOAD)
        self.batch_install_btn.clicked.connect(self.on_batch_install_clicked)
        progress_layout.addWidget(self.batch_install_btn)
        ...
        self._batch_queue = []  # 批量入库队列
        self._batch_running = False  # 是否正在批量入库
    ...
    def _request_cancel_download(self):
        ...
        if self.unlock_worker and self.unlock_worker.isRunning():
            ...
            # 批量入库时直接取消 asyncio 任务（quit 对 AsyncWorker 无效）
    ...
    def unlock_game_direct(self, appid, game_name):
        ...
        if hasattr(self, 'unlock_worker') and self.unlock_worker and self.unlock_worker.isRunning():
            ...
            if choice[0] == "cancel":
                ...
                new_worker = AsyncWorker(self._build_unlock_coro(appid, game_name))
        ...
        self.unlock_worker = AsyncWorker(self._build_unlock_coro(appid, game_name))
    ...
    def _selected_tool_type(self):
        """当前选择的清单源（仅在 GUI 线程调用）"""
        index_to_source = {
            0: "auto",
            1: "cysaw",
            2: "gmrc",
        ...
        return index_to_source.get(self.manifest_source_combo.currentIndex(), "auto")

    def _build_unlock_coro(self, appid, game_name, add_all_dlc=None, patch_depot_key=None, patch_manifest=None, tool_type=None):
        """构建单个游戏入库协程，返回 (success, source_used)

        选项参数在 GUI 线程读取后传入，避免在 worker 线程访问控件。
        """
        # 获取选项（默认从当前控件读取，须在 GUI 线程调用）
        if add_all_dlc is None:
            add_all_dlc = self.add_dlc_check.isChecked()
        if patch_depot_key is None:
        ...
        if tool_type is None:
            tool_type = self._selected_tool_type()
        ...
        async def _unlock():
            async with CaiBackend() as backend:
                ...
                # 入库成功后写入已入库记录（含 DLC appid，供去重使用）
                async def _record_success(source):
                    dlcs = []
                    if add_all_dlc:
                        try:
                            dlc_list = await asyncio.wait_for(backend.get_game_dlcs(str(appid)), timeout=10)
                            dlcs = [str(d.get('appid', '')) for d in dlc_list if str(d.get('appid', '')).isdigit()]
                        except Exception:
                            pass
                    _add_installed_record(appid, game_name, source, dlcs)
                    ...
                    for i, src in enumerate(auto_sources):
                        ...
                        if ok:
                            ...
                            if patch_manifest:
                                ...
                                post_result = await backend.complete_manifest_files(str(appid))
                                ok_result = bool(post_result.get("success", True))
                                if ok_result:
                                    await _record_success(src)
                                return (ok_result, src if ok_result else "")
                            else:
                ...
                if success:
                    if patch_manifest:
                        ...
                        ok_result = bool(post_result.get("success", True))
                        if ok_result:
                            await _record_success(tool_type_actual)
                        return (ok_result, tool_type_actual if ok_result else "")
                    else:
                        backend.log.info(f"主入库成功，跳过补全清单文件（未勾选修补Manifest）")
                        self._update_download_progress_state(58, "阶段 4/6：主入库完成，跳过补全清单文件")
                        await _record_success(tool_type_actual)
                        return (True, tool_type_actual)
    ...
    def on_unlock_complete(self, result):
        ...
        # 兼容旧返回：现在为 (success, source) 元组
        if isinstance(result, tuple):
            success, _source = result
        else:
            success, _source = result, ""
    ...
    def on_batch_install_clicked(self):
        """批量入库：按顺序逐个入库选中的游戏"""
        if self._batch_running:
            return
        if self.unlock_worker and self.unlock_worker.isRunning():
            InfoBar.info(
                title=tr("batch_install"),
                content=tr("installing_in_progress"),
                parent=self,
                position=InfoBarPosition.TOP,
                duration=2500
            )
        ...
        selected = [c for c in self.result_cards if getattr(c, 'checkBox', None) and c.checkBox.isChecked()]
        if not selected:
            InfoBar.warning(
                title=tr("batch_install"),
                content=tr("batch_no_selection"),
                parent=self,
                position=InfoBarPosition.TOP,
                duration=2500
            )
        ...
        for card in selected:
            if _get_existing_install_status(card.appid):
                skipped += 1
            else:
                pending.append(card)
        if skipped:
            InfoBar.warning(
                ...
                content=tr("batch_skipped", skipped),
                parent=self,
                position=InfoBarPosition.TOP,
                duration=2500
            )
        ...
        self._batch_queue = [(c.appid, c.game_name) for c in pending]
        self._batch_running = True
        self.batch_install_btn.setEnabled(False)
        self.select_all_check.setEnabled(False)
        self._start_download_progress(tr("batch_install"))

        # 在 GUI 线程读取入库选项，避免 worker 线程访问控件
        batch_opts = {
            "add_all_dlc": self.add_dlc_check.isChecked(),
            "patch_depot_key": self.patch_key_check.isChecked(),
            "patch_manifest": self.patch_manifest_check.isChecked(),
        ...
        async def _batch_loop():
            ...
            failed_names = []
            for i, (bid, bname) in enumerate(self._batch_queue):
                if self._download_cancel_requested:
                    break
                self._update_download_progress_state(
                    max(1, i * 95 // max(total, 1)),
                    f"{tr('batch_install')} {i + 1}/{total}：{bname}"
                )
                try:
                    ...
                    if ok:
                        ...
                        failed_count += 1
                        failed_names.append(bname)
                except Exception as e:
                    failed_count += 1
                    failed_names.append(f"{bname}（{e}）")
            return {"success": success_count, "failed": failed_count, "failed_names": failed_names}

        _replace_worker(getattr(self, 'unlock_worker', None))
        self.unlock_worker = AsyncWorker(_batch_loop())
        self.unlock_worker.result_ready.connect(self.on_batch_complete)
        self.unlock_worker.error.connect(self.on_batch_error)
        self.unlock_worker.finished.connect(self.unlock_worker.deleteLater)
        self.unlock_worker.finished.connect(self._on_batch_worker_finished)
        self.unlock_worker.start()

    def _on_batch_worker_finished(self):
        """批量 worker 结束兜底清理（含取消场景）"""
        if not self._batch_running:
            return
        self._batch_running = False
        # 仅当结束的 worker 就是当前引用的 worker 时才清引用，
        # 避免覆盖“换成当前清单入库”刚创建的新 worker
        sender = self.sender()
        if sender is None or sender is self.unlock_worker:
            self.unlock_worker = None
        self.batch_install_btn.setEnabled(True)
        self.select_all_check.setEnabled(True)
        self._finish_download_progress(False, tr("batch_failed"))
    ...
    def on_batch_complete(self, result):
        ...
        summary = tr("batch_done", success, failed)

        if failed:
            self._finish_download_progress(False, summary)
            detail = "\n".join(failed_names[:5]) if failed_names else ""
            InfoBar.error(
                title=tr("batch_failed"),
            ...
            self._finish_download_progress(True, summary)
            InfoBar.success(
                title=tr("batch_install"),
                content=summary,
                parent=self.window(),
...
class SettingsPage(ScrollArea):
    ...
    def _build_phase2(self):
        ...
        # 自动检查更新选项（信号在 _setup_auto_save_listeners 中连接）
        self.auto_update_check = SwitchButton()
        self.auto_update_check.setChecked(True)
        # 注意：信号连接移到 _setup_auto_save_listeners，避免配置加载前触发保存
        app_config_card.addGroup(FluentIcon.UPDATE, tr("auto_check_update"), tr("auto_check_update_hint"), self.auto_update_check)
    ...
    def _build_phase4(self):
        ...
        # 下一帧：备份与恢复卡片
        from PyQt6.QtCore import QTimer
        QTimer.singleShot(0, self._build_phase5)
    ...
    def _build_phase5(self):
        """第5帧：备份与恢复卡片"""
        layout = self._build_layout

        backup_card = GroupHeaderCardWidget(self)
        backup_card.setTitle(tr("backup_restore"))
        backup_card.setBorderRadius(8)

        self.backup_btn = PushButton(tr("backup"), self)
        self.backup_btn.setIcon(FluentIcon.SAVE)
        self.backup_btn.setFixedWidth(120)
        self.backup_btn.clicked.connect(self._on_backup_clicked)
        self.restore_btn = PushButton(tr("restore"), self)
        self.restore_btn.setIcon(FluentIcon.FOLDER)
        ...
        backup_card.addGroup(FluentIcon.FOLDER, tr("backup_restore"), tr("backup_hint"), btn_widget)
    ...
    def _on_backup_clicked(self):
        ...
        def _do_backup():
            ...
            with zipfile.ZipFile(path, 'w', zipfile.ZIP_DEFLATED) as zf:
                # 已入库记录
                rec_path = INSTALLED_RECORDS_PATH
                if rec_path.exists():
                    zf.write(str(rec_path), "installed_games.json")
                    count += 1
                if steam_path and steam_path.exists():
                    # 清单文件（保留相对目录前缀便于恢复）
                    for sub, prefix in (("depotcache", "depotcache"),
                                        ("config/depotcache", "config_depotcache"),
                                        ("appcache/depotcache", "appcache_depotcache")):
                        d = steam_path / sub
                        if d.exists():
                            try:
                                for f in sorted(d.glob("*.manifest")):
                                    zf.write(str(f), f"{prefix}/{f.name}")
                                    count += 1
                            except Exception:
                                pass
                    # Steam 登录配置
        ...
        _replace_worker(getattr(self, 'backup_worker', None))
        self.backup_worker = AsyncWorker(_backup())
        self.backup_worker.result_ready.connect(self._on_backup_complete)
    ...
    def _on_backup_complete(self, result):
        ...
        InfoBar.success(
            title=tr("backup_success"),
            content=tr("backup_success") + f"（{result.get('count', 0)} 个文件）",
            parent=self.window(),
            position=InfoBarPosition.TOP,
            duration=3000
        )
    ...
    def _on_restore_clicked(self):
        """恢复：从 zip 解压回对应位置（当前文件先留 .bak）"""
        from PyQt6.QtWidgets import QFileDialog
        path, _ = QFileDialog.getOpenFileName(self, tr("restore"), "", "ZIP 文件 (*.zip)")
        if not path:
            return

        dialog = MessageBox(
            tr("restore_confirm_title"),
            tr("restore_confirm_message"),
            self.window()
        )
        if not dialog.exec():
            return
        ...
        def _do_restore():
            ...
            # 恢复前备份当前文件（.bak）
            cfg_dir = INSTALLED_RECORDS_PATH.parent
            cfg_dir.mkdir(parents=True, exist_ok=True)
            rec_path = INSTALLED_RECORDS_PATH
            if rec_path.exists():
                shutil.copy2(str(rec_path), str(rec_path.with_suffix('.json.bak')))
            if steam_path:
                vdf = steam_path / 'config' / 'config.vdf'
                if vdf.exists():
                    shutil.copy2(str(vdf), str(vdf.with_suffix('.vdf.bak')))

            restored = 0
            with zipfile.ZipFile(path, 'r') as zf:
                for name in zf.namelist():
                    target = _zip_entry_target(name, steam_path, rec_path)
                    if target is None:
                        continue
                    try:
        ...
        _replace_worker(getattr(self, 'restore_worker', None))
        self.restore_worker = AsyncWorker(_restore())
        self.restore_worker.result_ready.connect(self._on_restore_complete)
    ...
    def _on_restore_complete(self, result):
        ...
        InfoBar.success(
            title=tr("restore_success"),
            content=tr("restore_success", result.get('count', 0)),
            parent=self.window(),
            position=InfoBarPosition.TOP,
            duration=3000
        )
    ...
    def _setup_auto_save_listeners(self):
        ...
        if self.auto_update_check:
            self.auto_update_check.checkedChanged.connect(self._on_setting_changed)
    ...
    def on_config_loaded(self, config):
        ...
        if config:
            ...
            # 加载自动检查更新选项
            if self.auto_update_check:
                new_val = config.get("auto_check_update", True)
                if self.auto_update_check.isChecked() != new_val:
                    try:
                        self.auto_update_check.checkedChanged.disconnect(self._on_setting_changed)
                    except:
                        pass
                    self.auto_update_check.setChecked(new_val)
                    self.auto_update_check.checkedChanged.connect(self._on_setting_changed)
    ...
    def save_settings(self):
        ...
        async def _save():
            ...
            # 保存自动检查更新选项
            if self.auto_update_check:
                config["auto_check_update"] = self.auto_update_check.isChecked()
...
class MainWindow(MSFluentWindow):
    ...
    def _auto_check_update(self):
        """启动时自动检查更新（设置中关闭则跳过；静默模式，仅在有更新时提示）"""
        try:
            config_path = APP_ROOT / 'config' / 'config.json'
            if config_path.exists():
                import json
        ...
        _replace_worker(getattr(self, '_auto_update_worker', None))
        self._auto_update_worker = AsyncWorker(_check())
        ...
        def on_result(result):
            ...
            if has_update:
                ...
                msg.cancelButton.setText(tr("cancel"))
                if msg.exec():
                    download_url = self.settings_page._get_mirror_download_url(
                        info.get('release_url', f"https://github.com/{GITHUB_REPO}/releases"),
                        info.get('latest_version', '')
                    )
                    QDesktopServices.openUrl(QUrl(download_url))
        ...
        self._auto_update_worker.result_ready.connect(on_result)
        self._auto_update_worker.error.connect(on_error)
